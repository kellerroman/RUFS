...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   i2 =block (b) % nPkt(1)
   if (block(b) % face(f) % conType == BC_WALL) then
      do i = block (b) % nCell(1) +1,block (b) % nCell(1) + n_BC_Cells
         i1 = block(b) % nCell(1) - (i - block(b) % nCell(1) - 1)
!         i1 = i-1
         do j = 1 , block(b) % nCell(2)
            j1 = j
            j2 = j
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   j2 = block(b) % nPkt(2)
   if (block(b) % face(f) % conType == BC_WALL) then
      do j = block(b) % nCell(2)+1,block(b) % nCell(2)+n_BC_Cells
         j1 = block(b) % nCell(2) - (j - block(b) % nCell(2) - 1)
         do i = 1 , block(b) % nCell(1)
            i2 = i
            i1 = i
            block(b) % Q (i,j,k,1) = block(b) % Q (i1,j1,k1,1)
